...
import { portraitDocumentSchema, portraitReviewSchema, portraitSampleFrames, validatePortraitGeometry } from './portrait-schema.ts';
...
export function assertPortraitForPlan(input: unknown, plan: Plan) {
  const doc = portraitDocumentSchema.parse(input), errors = validatePortraitGeometry(doc.recipe);
  ...
  if (!doc.review || doc.review.recipeSha256 !== portraitHash(doc) || doc.review.decision !== 'accept' ||
      new Set(doc.review.checks.map(c=>c.dimension)).size !== 3 || doc.review.checks.some(c=>c.outcome!=='pass')) errors.push('Exact portrait framing review is required');
...
export function checkPortrait(input: unknown, context: ClipContext, geometry: {width:number;height:number}) {
  const doc = portraitDocumentSchema.parse(input), errors = validatePortraitGeometry(doc.recipe), needs: string[] = [];
  ...
  if (!doc.review) needs.push('Record the exact framing review after inspection');
    ...
    if (doc.review.recipeSha256 !== recipeSha256) errors.push('Portrait review is stale');
    if (new Set(doc.review.checks.map(c=>c.dimension)).size!==3) errors.push('Each framing review dimension must appear once');
    if (doc.review.decision==='reject' || doc.review.checks.some(c=>c.outcome==='fail')) needs.push('Framing review rejected or failed');
  }
...
export function recordPortraitReview(input: unknown, context: ClipContext, geometry: {width:number;height:number}, reviewInput: unknown) {
  const doc = portraitDocumentSchema.parse(input), review = portraitReviewSchema.parse(reviewInput);
  if (review.recipeSha256 !== portraitHash(doc)) throw new Error('Review does not bind this exact portrait recipe');
  doc.review = review;
  const check = checkPortrait(doc,context,geometry);
